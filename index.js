...
{
(async function () {
var legenda = q ? q.split("/")[0] : `╼⊳⊰ Solicitado Por: ${pushname}\n╼⊳⊰ Bot: ${botName}\n╼⊳⊰ Dono: ${donoName}`;
var autor = q && q.includes("/") ? q.split("/")[1] : "";
...
case 'fazerlink':
case 'gerarlink4': {
  // Verifica se há uma mídia (imagem, vídeo ou áudio) citada ou enviada
  const YSG = isQuotedVideo || isQuotedImage || isQuotedAudio || isImage || isVideo;
  
  // Se não houver, envia uma mensagem de erro
  if (!YSG) {
    reagir(from, "⚠️");
  ...
  // Função para fazer o upload do arquivo usando fetchJson
  async function upload(midia) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Usando fetchJson para enviar para o Catbox e obter o link
        const res = await fetchJson('https://catbox.moe/user/api.php', {
          method: 'POST',
          body: form
        });

        resolve(res.trim()); // Retorna o link
      } catch (erro) {
        reject(erro); // Em caso de erro, rejeita
      }
    });
  }

  // Função para processar a mídia e gerar o link
  async function processarMidia(tipo, mensagem, citado) {
    const boij = citado ? JSON.parse(JSON.stringify(info).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo.message[tipo] : mensagem[tipo];
    const owgi = await getFileBuffer(boij, tipo);
    await sleep(1000);
    reagir(from, "🔗");
  ...
  // Verifica o tipo de mídia e o tamanho para fazer o upload
...
case 'aptoide':
  ...
  try {
    // Pesquisar o app no Aptoide usando fetchJson
    const aptoide = await fetchJson(`https://ws75.aptoide.com/api/7/apps/search?query=${encodeURIComponent(q)}&trusted=true`);
    
    // Caso não encontre o app, retorna uma mensagem
    if (aptoide.data.datalist.total === 0) {
      return reply('• Não encontrei o aplicativo que você está pesquisando!');
    }

    // Função para formatar a descrição do aplicativo
    const description = (getApk, sizeApk) => `
      ꪶ͢͜𝐋𝐀𝐃𝐘 𝐁𝐎𝐓 - 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃‍ꫂ
    ...
    const photoAptoideBuffer = await fetchJson(aptoide.data.datalist.list[0].graphic);
    
    // Enviar a imagem com as informações
    loli.sendMessage(from, {
      image: photoAptoideBuffer,
      caption: description(aptoide.data.datalist.list[0], (aptoide.data.datalist.list[0].size / 1048576).toFixed(1))
    });

    // Enviar o arquivo APK do aplicativo
    loli.sendMessage(from, {
      document: { url: aptoide.data.datalist.list[0].file.path },
      mimetype: 'application/vnd.android.package-archive',
      fileName: `${aptoide.data.datalist.list[0].name}.apk`,
      caption: 'Aqui está o aplicativo: ' + q
    }, { quoted: info });
...
case 'news':
    ...
    try {
        // Usando fetchJson para acessar a API de vídeos
        const data = await fetchJson(`https://tohka.online/api/dl/PortalZacariasDlAll?apikey=Xd2Zdpju3M`);

        if (!data || data.status !== "operando") {
            return reply("🚨 API está fora do ar ou houve um erro ao acessar.");
        }
...
case 'zacarias_noticias': {
    try {
        // Usando fetchJson para acessar a API de notícias
        const data = await fetchJson(`https://tohka.online/api/pesquisa/portalzacarias?apikey=Xd2Zdpju3M`);
...
case 'esporte': 
  try {
    // Usando fetchJson para obter as notícias de esportes
    const data = await fetchJson('https://tohka.online/api/outros/globoesportes?apikey=Xd2Zdpju3M');
    ...
    if (data && data.length > 0) {
      ...
      // Envia a mensagem com as notícias de esportes
...
case 'temporal': {
    try {
        ...
        let city = q;
        let data = await fetchJson(`https://wttr.in/${city}?format=j1`);
...
case 'feriados':
    ...
    try {
        // Usando fetchJson para obter os feriados do ano desejado
        const feriados = await fetchJson(`https://api.invertexto.com/v1/holidays/${q}?token=5980|9n1NZ449fdM1Hxs8K8urHxDZRVfEznwI&state=br`);

        let mensagem = `*• Feriados para o ano de ${q}*:\n\n`;

        // Verifica se os feriados foram retornados corretamente
        if (feriados && feriados.length > 0) {
            feriados.forEach(({ date, name, type }) => {
                mensagem += `*• Data:* _${date}_\n*• Feriado:* _${name}_\n*• Tipo:* _${type}_\n*—*\n`;
            });
            loli.sendMessage(from, { text: mensagem }, { quoted: info });
        } else {
            reply('Não foram encontrados feriados para o ano informado.');
        }

    } catch (error) {
        console.error('Erro ao buscar os feriados:', error);
        reply(`Erro ao obter a lista de feriados.`);
    }
    break;
...
case 'deviantart':
    ...
    try {
        ...
        // Usando fetchJson para buscar os dados da pesquisa
        ...
        // Encontrar imagens dentro da resposta
        ...
        for (const img of images.slice(0, 5)) {
            ...
            await sleep(1000);  // Atrasar para não sobrecarregar o bot
        }
    } catch (e) {
...
case 'pinterestvideo': case 'pinterest5': case 'Pinterestvideo': {
    try {
        ...
        const response = await fetchJson(`https://tohka.online/api/pesquisa/pinterestvideo?nome=${encodeURIComponent(q)}&apikey=Xd2Zdpju3M`);
        if (!response || !response.length) return reply("Erro ao buscar os dados do Pinterest.");
...
case 'encrypt':
    if (!q) {
        ...
        try {
            ...
            // Usando fetchJson para fazer a requisição da API
...
case 'ff':
  ...
  case 'gitclone':
    ...
    try {
        // Usando fetchJson para obter a resposta da API do GitHub
        const headers = await fetchJson(url, { method: 'HEAD' });
        ...
        console.error('Erro ao baixar o repositório:', e);
        return reply('Erro ao tentar clonar o repositório. Tente novamente mais tarde.');
    }
    break;
...
case 'pinterest':
    ...
    try {
        const urls = await fetchJson(pinterestApiUrl);
...
case 'pinterest2':
    ...
    try {
        const urls = await fetchJson(apiUrlPinterest2);
...
case 'xvideos': {
    ...
    const searchTerm = q.trim() || 'ia';  // Usar o termo fornecido, ou 'ia' por padrão
    const urlXvideos = `http://tohka.tech/api/pesquisa/xvideos?nome=${encodeURIComponent(searchTerm)}&apikey=Xd2Zdpju3M`;
...
case 'pornogratis': {
    ...
    // O nome da pesquisa vai ser dinâmico
    const searchTerm = q.trim() || 'orgasm';  // Usar o termo de pesquisa fornecido, ou "orgasm" como padrão
    const urlPornogratis = `https://tohka.online/api/pesquisa/pornogratis?nome=${encodeURIComponent(searchTerm)}&apikey=Xd2Zdpju3M`;
    reply("🔄 Realizando a pesquisa no Pornogratis...");

    try {
        const resultPornogratis = await fetchJson(urlPornogratis);
...
case 'pornohub': {
  ...
  const urlPornhubsrc = 'https://tohka.online/api/pesquisa/pornhubsrc?nome=orgasm&apikey=Xd2Zdpju3M';
...
case "play4": {
  ...
  try {
    ...
    // Se a resposta contiver dados válidos
    if (!response.data || response.data.length === 0) {
      return loli.sendMessage(from, { text: "Erro: Não foi possível obter o vídeo solicitado." }, { quoted: info });
    }

    // Enviar o arquivo de vídeo para o WhatsApp
    await loli.sendMessage(
      from,
      {
        video: Buffer.from(response.data), // Enviar o vídeo como buffer
    ...
    // Se preferir enviar o link para um player externo (se disponível), poderia fazer algo assim:
    // const videoLink = 'https://www.youtube.com/watch?v=' + getVideoId(text); 
    // return loli.sendMessage(from, { text: `🎬 Aqui está o vídeo: ${videoLink}` }, { quoted: info });
...
case 'frases': {
    try {
        ...
        const data = await fetchJson(`https://world-ecletix.onrender.com/api/pensador?nome=${encodeURIComponent(q)}`);
        ...
        data.forEach((fraseInfo) => {
            message += `*Frase:* ${fraseInfo.frase}\n` +
                       ...
                       (fraseInfo.image ? `*Imagem:* ${fraseInfo.image}\n` : ''); // Verificação de imagem
        });
...
case 'verificarban': {
    if (!q.trim()) {
        return reply("Você precisa fornecer um ID para verificar o banimento.");
    }

    try {
        const response = await fetchJson(`https://world-ecletix.onrender.com/api/banido?id=${encodeURIComponent(q)}`);
        
        if (!response || response.status !== "success") {
            return reply("Erro ao verificar banimento.");
        }

        const statusBan = response.isBanned === "yes" ? "🚨 *Usuário BANIDO!* 🚨" : "✅ *Usuário não está banido.*";

        return reply(`🔍 *Verificação de Banimento*\n🆔 *ID:* ${response.uid}\n${statusBan}`);
    } catch (error) {
        console.error("Erro ao processar a solicitação:", error);
        return reply("Ocorreu um erro ao verificar o banimento. Tente novamente.");
    }
}
break;
...
case 'ffeventos': {
    try {
        const response = await fetchJson(`https://world-ecletix.onrender.com/api/eventos`);
        ...
        response.events.forEach(evento => {
        ...
        console.error("Erro ao processar a solicitação:", error);
        return reply("Ocorreu um erro ao buscar os eventos do Free Fire. Tente novamente.");
    }
}
break;
  case 'twitter':
...
case 'hentai_neko': {
    ...
    try {
        const response = await fetchJson('https://tohka.online/api/hentai/random?categoria=neko&apikey=Xd2Zdpju3M');
...
case 'hentai_ass': {
    ...
    try {
        const response = await fetchJson('https://tohka.online/api/hentai/ass?apikey=Xd2Zdpju3M');
...
case 'hentai_milf': {
    ...
    try {
        const response = await fetchJson('https://tohka.online/api/hentai/milf?apikey=Xd2Zdpju3M');
...
case 'hentaizinho': {
    ...
    try {
        const response = await fetchJson('https://tohka.online/api/hentai/hentai?apikey=Xd2Zdpju3M');
...
case 'hentai_paizuri': {
    ...
    try {
        const response = await fetchJson('https://tohka.online/api/hentai/paizuri?apikey=Xd2Zdpju3M');
...
case 'hentai_oral': {
    ...
    try {
        const response = await fetchJson('https://tohka.online/api/hentai/oral?apikey=Xd2Zdpju3M');
...
case 'hentai_ecchi': {
    ...
    try {
        const response = await fetchJson('https://tohka.online/api/hentai/ecchi?apikey=Xd2Zdpju3M');
...
case 'hentai_ero': {
    ...
    try {
        const response = await fetchJson('https://tohka.online/api/hentai/ero?apikey=Xd2Zdpju3M');
...
case 'gerarimagem':
    ...
    try {
        ...
        if (response.data && response.data.length > 0) {
            const urlImagem = response.data[0].url;
...
case 'gerarimg': // by luanzin dev
  ...
  try {
    const response = await fetchJson(`https://api-ia.orbital.host/orbital/img-ai?prompt=${promptGerarImg}&model=v3`);
...
case 'sabetudo':
  ...
  try {
    // Faz a requisição para a nova API do SabeTudo usando fetchJson
    const data = await fetchJson(`https://world-ecletix.onrender.com/api/sabetudo?texto=${pergunta}`);
    
    // Verifica se a resposta contém a chave 'resposta' e envia a resposta
    if (data.resposta) {
      // Verifica se a resposta não contém bloqueio
      if (data.resposta.includes("natureza sexualmente sugestiva")) {
        return reply("🚨 A pergunta não pode conter conteúdo de natureza sexualmente sugestiva. Tente novamente com outra pergunta.");
      }
      // Se a resposta for válida, envia a resposta
      loli.sendMessage(sender, { text: `🗣️ Resposta do SabeTudo: ${data.resposta}` }, { quoted: info });
    } else {
      reply("🚨 Não consegui obter uma resposta. Tente novamente mais tarde.");
    }
  } catch (error) {
