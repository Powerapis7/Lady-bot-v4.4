...
if (isGroup && groupIdscount.indexOf(from) >= 0) {
    ...
    if (numbersIds.indexOf(sender) >= 0) {
        ...
        countMessage[ind].numbers[indnum].cmd_messages += 1;
...
reply('Traduzindo o texto solicitado...');
txt = q.replace(" |", "|").replace("| ", "|").replace(" | ", "|")
var [idiomas, textos] = txt.split("|")
if(!q.includes("|")) return reply(`Está faltando a |, *use como exemplo:* ${prefix}tradutor pt|Dog`)

let tradutor = await fetchJson(`https://carisys.online/api/outros/tradutor?idioma=${encodeURI(idiomas)}&query=${encodeURI(textos)}`)

await loli.sendMessage(from, 
{text: `Aqui esta o seu texto traduzido:\n➩ ${tradutor.resultado}`}, 
{quoted: info})
...
break;
 case 'hentai':
  const hentais = await fetchJson('https://carisys.online/api/outros/hentais/videos');
  const video = hentais?.resultado?.[Math.floor(Math.random() * hentais.resultado.length)];
  video ? await loli.sendMessage(from, { video: { url: video.videoUrl }, caption: `🎬 *Título:* ${video.titulo}\n📌 *Categoria:* ${video.categoria}\n🔄 *Compartilhamentos:* ${video.compartilhamentos}\n👀 *Views:* ${video.views}\n🔗 *Confira:* ${video.url}` }, { quoted: selozeus }) : reply('Parece que não encontramos nenhum vídeo no momento!');
break
 case 'toanime': 
case 'imagepraanime':
if((isMedia && !info.message.videoMessage || isQuotedImage)) {
...
await loli.sendMessage(from, {image: 
...
case 'tiktoksearch':
try {
if (!q) return reply('cade o nome da pesquisa?')
nazu.react(['📌']); 
nex = await fetchJson(`https://api.nexfuture.com.br/api/pesquisas/tiktok/search?query=${encodeURIComponent(q)}`)

await loli.sendMessage(from, {video: 
{url: nex.resultado.videos}, 
caption: nex.resultado.titulo, mimetype: "video/mp4"}, {quoted: info})
} catch (error) {
console.log(error)
return reply('Ocorreu um erro, tente novamente mais tarde.')
}
...
case 'kwaistalk':
try {
if (!q) return reply(`digite o username da pessoa que você deseja stalkear no kwai.\nexemplo: /kwaistalk Vegeta_Sola`)
reply('Stalkeando usuário solicitado.')

kwstalk = await fetchJson(`https://api.nexfuture.com.br/api/outros/kwstalk?query=${encodeURIComponent(q)}`);

let kstalk = `
✏️ ɴᴏᴍᴇ ᴏʀɢ: ${kwstalk.resultado.nome_usuario}
😍 ɴᴏᴍᴇ: ${kwstalk.resultado.nome}
📌 ɪᴅ: ${kwstalk.resultado.id_usuario}
❤ ᴄᴜʀᴛɪᴅᴀs: ${kwstalk.resultado.curtidas}
📡 sᴇɢᴜɪᴅᴏʀᴇs: ${kwstalk.resultado.seguidores}
🎉 sᴇɢᴜɪɴᴅᴏ: ${kwstalk.resultado.seguindo}
💌 ʙɪᴏ: ${kwstalk.resultado.bio}
💎 ᴘᴇʀғɪʟ: ${kwstalk.resultado.url_perfil}
` 
await loli.sendMessage(from, 
{image: {url: kwstalk.resultado.foto_perfil},
caption: kstalk}, {quoted:info});
} catch (error) {
console.log(error)
return reply('Ocorreu um erro ao buscar o usuário,verifique o user se está correto!');
}
break
case 'kwai_vid':
case 'kwaidl':
if (!q) return reply('Cadê a URL do vídeo do Kwai ?')
try {
kwai = await fetchJson(`https://api.nexfuture.com.br/api/downloads/kwai/dl?url=${encodeURIComponent(q)}`);

let kdl = `
🤯 ᴛɪ́ᴛᴜʟᴏ: ${kwai.resultado.titulo || "Desconhecido"}
👨🏻‍✈️ ᴀᴜᴛᴏʀ: ${kwai.resultado.criador.nome || "Não disponível"}
🎊 ᴜsᴇʀ: ${kwai.resultado.criador.usuario || "Não disponível"}
📨 ᴘᴜʙʟɪᴄᴀᴅᴏ: ${kwai.resultado.publicado || "Não disponível"}
⏰ ᴅᴜʀᴀᴄ̧ᴀ̃ᴏ: ${kwai.resultado.duracao || "Não disponível"}

📝 ᴅᴇsᴄʀɪᴄ̧ᴀ̃ᴏ: ${kwai.resultado.descricao || "Não disponível"}`;

await loli.sendMessage(from, {video: 
{url: kwai.resultado.video}, caption: kdl},
{quoted: info });
} catch (error) {
console.log(error);
return reply('Ocorreu um erro ao baixar o vídeo. Tente novamente mais tarde.');
}
break
case 'mediafire':
try {
...
case 'gerarlink4':
  ...
  if (!YSG) {
    reagir(from, "⚠️");
    reply(`⚠️ Você precisa marcar uma imagem, vídeo ou áudio de até 200MB com o comando ou enviar uma imagem ou vídeo e colocar na legenda: \`${prefixo + command}\`.`);
    return;
  }
  ...
  async function processarMidia(tipo, mensagem, citado) {
    boij = citado ? JSON.parse(JSON.stringify(info).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo.message[tipo] : mensagem[tipo];
    owgi = await getFileBuffer(boij, tipo);
    await sleep(1000);
    reagir(from, "🔗");
  ...
  if ((isMedia && info.message.imageMessage?.fileLength < 209715200) || 
      (isQuotedImage && info.message.extendedTextMessage.contextInfo.quotedMessage.imageMessage?.fileLength < 209715200)) {
    processarMidia('imageMessage', info.message, isQuotedImage);
  } 
  else if ((isMedia && info.message.videoMessage?.fileLength < 209715200) || 
           (isQuotedVideo && info.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage?.fileLength < 209715200)) {
    processarMidia('videoMessage', info.message, isQuotedVideo);
  } 
  else if (isQuotedAudio && info.message.extendedTextMessage.contextInfo.quotedMessage.audioMessage?.fileLength < 209715200) {
    processarMidia('audioMessage', info.message, isQuotedAudio);
  } 
  else {
    reagir(from, "❗");
...
┗━━━〔‼️〕━━━━
           ...
           await loli.sendMessage(from, {image: fs.readFileSync('./loli/image/menu.jpg'), caption: menu8, contextInfo: {
            ...
            sourceUrl: `https://instagram.com/luanzn.fe`, 
            mediaType: 2
            }}})
...
case 'vasconews': 
    ...
    break
    case 'zacarias_videos': {
    try {
        const response = await fetch(`https://tohka.online/api/dl/PortalZacariasDlAll?apikey=Xd2Zdpju3M`);
        if (!response.ok) return reply("❌ Erro ao acessar a API de vídeos.");

        const data = await response.json();

        if (data.status !== "operando") {
            return reply("🚨 API está fora do ar no momento.");
        }

        if (!data.resultado || data.resultado.length === 0) {
            return reply("⚠️ Nenhum vídeo disponível no momento.");
        }

        let mensagem = "🎥 *Últimos Vídeos do Portal Zacarias:* \n\n";
        data.resultado.slice(0, 10).forEach((item, index) => { // Limita a 10 vídeos
            mensagem += `📹 *Vídeo ${index + 1}:* [🔗 Baixar Vídeo](${item.video})\n`;
        });

        return reply(mensagem.trim());
        
    } catch (e) {
        console.log('Erro ao buscar vídeos:', e);
        return reply("❌ Erro ao tentar buscar os vídeos.");
    }
}
break;

case 'zacarias_noticias': {
    try {
        const response = await fetch(`https://tohka.online/api/pesquisa/portalzacarias?apikey=Xd2Zdpju3M`);
        if (!response.ok) return reply("❌ Erro ao buscar as notícias.");

        const data = await response.json();
        if (!data || data.length === 0) {
            return reply("⚠️ Nenhuma notícia encontrada no momento.");
        }

        let noticiasMsg = `📰 *Últimas Notícias do Portal Zacarias:* \n\n`;
        data.slice(0, 5).forEach((noticia, index) => { // Mostra as 5 primeiras notícias
            noticiasMsg += `🔹 *${index + 1}. ${noticia.titulo}*\n` +
                           `🔗 [Leia mais](${noticia.link})\n\n`;
        });

        return reply(noticiasMsg.trim());
    } catch (e) {
        console.log('Erro ao buscar notícias:', e);
        return reply("❌ Erro ao tentar buscar as notícias.");
    }
}
break;
case 'cnnbrasil':
...
case 'esporte': 
  try {
    const response = await fetch('https://tohka.online/api/outros/globoesportes?apikey=Xd2Zdpju3M');
    const data = await response.json();
...
case 'pergunta':
try {
ABC = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/PERGUNTE_E_EU_RESPONDO?q=${q.trim()}&apikey=OrbitalCloud`)
reply(`( ${ABC.msg} )`)
} catch { 
reply("Erro..")
}
...
ABC = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/google-img?nome=${q}&apikey=OrbitalCloud`);
...
try {
ABC = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/esporte_noticias?apikey=OrbitalCloud`)
AB =""
...
data = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/playstore?nome=${q}&apikey=OrbitalCloud`)
...
ABC = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/info_celular?celular=${q}&apikey=OrbitalCloud`);
...
try {
reply("Aguarde..")
ABC = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/Amazon_Pesquisa?pesquisa=${q.trim()}&apikey=OrbitalCloud`)
RST = "Pesquisa Amazon:\n\n"
...
AB = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pesquisa_ytb?nome=${q}&apikey=OrbitalCloud`)
ABC = `${"-\t".repeat(13)}\n\n`
for (var i of AB) {
ABC += `Titulo: ${i.titulo}\nUrl: ${i.url}\nTempo: ${i.tempo}\nPostado: ${i.postado}\n\nDescrição: ${i.desc}\n\n`;
ABC += `${"-\t".repeat(13)}\n\n`
...
try {
loli.sendMessage(from, {image: {url: `https://api.bronxyshost.com.br/api-bronxys/print_de_site?url=${q.trim()}&apikey=OrbitalCloud`}}, {quoted: info}) 
} catch {
return reply("Erro..");
}
break;
...
fetch(`https://api.bronxyshost.com.br/transcrever?apikey=OrbitalCloud`, {
method: 'POST',
body: formData
...
case 'attp6': case 'attp7': case 'attp8': case 'attp9': case 'attp10':
try {
    if (!q || !q.trim()) return reply(`Exemplo: ${prefix + command} lady`);
    ...
    loli.sendMessage(from, { 
        sticker: { 
            url: `https://api.bronxyshost.com.br/api-bronxys/attp_edit?texto=${encodeURIComponent(q)}&fonte=${encodeURIComponent(Fontes)}&apikey=OrbitalCloud` 
        } 
    }, { quoted: info }).catch(() => {
...
data = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pesquisa_ytb?nome=${q}&apikey=OrbitalCloud`)
...
loli.sendMessage(from, {audio: {url: `https://api.bronxyshost.com.br/api-bronxys/play?nome_url=${q}&apikey=OrbitalCloud`}, mimetype: "audio/mpeg", fileName: data[0]?.titulo || "play.mp3"}, {quoted: info}).catch(e => {
return reply("Erro..")
})
} catch (e) {
console.log(e)
...
`
loli.sendMessage(from, {image: {url: data[0]?.thumb || logoslink?.logo}, caption: bla}, {quoted: info})
loli.sendMessage(from, {video: {url: `https://api.bronxyshost.com.br/api-bronxys/play_video?nome_url=${q}&apikey=${API_KEY_BRONXYS}`}, mimetype: "video/mp4", fileName: data[0]?.titulo || "play.mp4"}, {quoted: info}).catch(e => {
return reply("Erro..")
})
} catch (e) {
console.log(e)
...
`
    loli.sendMessage(from, { image: { url: data[0]?.thumb || logoslink?.logo }, caption: bla }, { quoted: info })
    loli.sendMessage(from, { audio: { url: `https://api.bronxyshost.com.br/api-bronxys/play?nome_url=${q}&apikey=OrbitalCloud` }, mimetype: "audio/mpeg", fileName: data[0]?.titulo || "play.mp3", ptt: true }, { quoted: info })
      .catch(e => {
        return reply("Erro..")
      })
  } catch (e) {
...
try {
reply("Aguarde...")
const abc = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/letra_musica?letra=${q.trim()}&apikey=OrbitalCloud`)
...
reply("Realizando ação..")
loli.sendMessage(from, {video: {url: `https://api.bronxyshost.com.br/api-bronxys/${command}?url=${q}&apikey=OrbitalCloud`}, mimetype: "video/mp4"}).catch(e => {
return reply("Erro..")
})
} catch (e) {
return reply("Erro..")
...
if(!q.includes("facebook") && !q.includes("fb.watch")) return reply(`Exemplo: ${prefix+command} o link do Facebook`);
reply("Realizando ação..")
loli.sendMessage(from, {audio: {url: `https://api.bronxyshost.com.br/api-bronxys/${command}?url=${q}&apikey=OrbitalCloud`}, mimetype: "audio/mpeg"}).catch(e => {
return reply("Erro..")
})
} catch (e) {
return reply("Erro..")
...
try {
loli.sendMessage(from, {video: {url: `https://api.bronxyshost.com.br/api-bronxys/threads?url=${q}&apikey=OrbitalCloud`}})
} catch (e) {
...
reply(Res_Aguarde);
try {
loli.sendMessage(from, {video: {url: `https://api.bronxyshost.com.br/api-bronxys/kwai?url=${q.trim()}&apikey=OrbitalCloud`}, mimetype: "video/mp4"}, {quoted: info})
} catch (e) {
console.log(e);
return reply("Erro...");
}
...
ABC = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/instagram?url=${q.trim()}&apikey=OrbitalCloud`)
reply("Realizando ação..")
...
case 'seuanimal': // Feito por JP
    try {
        const animais = [
            "🦁 Leão 🦁", "🐯 Tigre 🐯", "🐺 Lobo 🐺", "🦊 Raposa 🦊", "🐻 Urso 🐻", 
            "🐵 Macaco 🐵", "🐸 Sapo 🐸", "🦅 Águia 🦅", "🐴 Cavalo 🐴", "🐘 Elefante 🐘",
            "🐍 Cobra 🐍", "🦉 Coruja 🦉", "🐬 Golfinho 🐬", "🐱 Gato 🐱", "🐶 Cachorro 🐶"
        ];

        const animalEscolhido = animais[Math.floor(Math.random() * animais.length)];

        const mensagemAnimal = `
🦄✨ *QUAL ANIMAL VOCÊ SERIA?* ✨🦄
━━━━━━━━━━━━━━━
🐾 Você seria um(a): *${animalEscolhido}* 
━━━━━━━━━━━━━━━
😂 *Combina contigo ou não?* 🤣
        `;
        reply(mensagemAnimal);
    } catch (erro) {
        console.error(erro);
        reply("❌ *Deu erro, tenta de novo!* ❌");
    }
    break;

case 'tamanhodopau':
case 'pika': // Feito por JP
    try { 
        // Gerando um tamanho aleatório entre 1 e 10
        const tamanhos = ["👶 Pequena 🍼", "👦 Normal 🏋️", "🧔 Grande 💪", "🐉 Gigante 🐲", "🌌 Extravagante 🌟"];
        const tamanhoEscolhido = tamanhos[Math.floor(Math.random() * tamanhos.length)];

        const mensagemTamanho = `
🌟✨ *QUAL O TAMANHO DA SUA PIKA?* ✨🌟
━━━━━━━━━━━━━━━
💫 O tamanho da sua pika é: *${tamanhoEscolhido}*
━━━━━━━━━━━━━━━
😄 *Combina com você?* 😂
        `;
        reply(mensagemTamanho); 
    } catch (erro) { 
        console.error(erro); 
        reply("❌ *Deu erro, tenta de novo!* ❌"); 
    } 
...
🌍 *Google Maps:* [Clique aqui](https://www.google.com/maps?q=${latitude},${longitude})
        ...
        await loli.sendMessage(from, { text: mensagemGPS }, { quoted: info });
...
case 'suasorte': // Feito por JP
    try {
        ...
        const sorteSorteada = sortes[Math.floor(Math.random() * sortes.length)];

        const mensagemSorte = `
${sorteSorteada.simbolo} *PREVISÃO DO SEU DIA!* ${sorteSorteada.simbolo}
━━━━━━━━━━━━━━━
🔮 *Mensagem:* ${sorteSorteada.mensagem}
━━━━━━━━━━━━━━━
...
🔹 *Link Completo:* [Clique Aqui](${link})
        ...
        if (imagem) {
            ...
            await loli.sendMessage(from, { image: buffer, caption: mensagemReceita }, { quoted: info });
        } else {
            await loli.sendMessage(from, { text: mensagemReceita }, { quoted: info });
        }
    } catch (erro) {
...
case 'desafio': // Feito por JP
    try {
        const desafios = [
            "💪 Fale 𝟱 𝗽𝗮𝗹𝗮𝘃𝗿𝗮𝘀 𝘀𝗲𝗺 𝘂𝘀𝗮𝗿 𝗮 𝗹𝗲𝘁𝗿𝗮 '𝗮'! 🔤😂",
            "🔥 Fique 𝟭 minuto 𝘀𝗲𝗺 𝗿𝗶𝗿! 😆⏳",
            "🤯 Envie um emoji que represente seu humor agora! 🤔🎭",
            "👀 Digite seu nome ao contrário! 🔄😂",
            "🚀 Conte até 𝗱𝗲𝘇 𝗲𝗺 𝘂𝗺𝗮 𝗹í𝗻𝗴𝘂𝗮 𝗾𝘂𝗲 𝘃𝗰 𝗻ã𝗼 𝘀𝗮𝗯𝗲! 🌍🤔"
        ];

        const desafioSorteado = desafios[Math.floor(Math.random() * desafios.length)];

        const mensagemDesafio = `
📢 *DESAFIO DO DIA!* 🎯  
━━━━━━━━━━━━━━━━━━  
${desafioSorteado}  
━━━━━━━━━━━━━━━━━━  
💡 *Consegue completar essa bosta?* 😏  
        `;

        reply(mensagemDesafio);
    } catch (erro) {
        console.error(erro);
        reply("❌ *Deu ruim no desafio, mano!* ❌");
    }
    break;
case 'seucarro': // jp
    try {
        const listaCarros = [
            { nome: "Fusca", emoji: "🚗" },
            { nome: "Civic", emoji: "🚙" },
            { nome: "Mustang", emoji: "🏎️" },
            { nome: "Ferrari", emoji: "🚘" },
            { nome: "Lamborghini", emoji: "🚀" },
            { nome: "Kombi", emoji: "🚌" },
            { nome: "BMW", emoji: "🚙" },
            { nome: "Porsche", emoji: "🚗" },
            { nome: "Gol", emoji: "🚗" },
            ...
            { nome: "Tesla", emoji: "🔋" },
            { nome: "Audi", emoji: "🚘" },
            { nome: "McLaren", emoji: "🏎️" },
            { nome: "Jeep", emoji: "🚙" },
            { nome: "Corvette", emoji: "🚗" },
            { nome: "Chrysler 300C", emoji: "🚓" },
            { nome: "Pajero", emoji: "🚙" },
            { nome: "Camaro", emoji: "🏁" },
            { nome: "Fiat Uno", emoji: "🚗" },
            { nome: "Chevrolet Tracker", emoji: "🚙" }
        ];

        const carroSorteado = listaCarros[Math.floor(Math.random() * listaCarros.length)];

        const mensagemCarro = `
🚗 *SEU CARRO É:* 🚗
━━━━━━━━━━━━━━━━━━━━
🛣️ *Modelo:* ${carroSorteado.nome} ${carroSorteado.emoji}
💨 *Agora é só acelerar e sair por aí, cuidado com o radar!*
━━━━━━━━━━━━━━━━━━━━
😂 *Seu carro é veloz ou só no meme mesmo?* 😜
        `;
        reply(mensagemCarro);
    } catch (erro) {
        console.error(erro);
...
case 'suafrase': // jp
    try {
        const listaFrases = [
            { texto: "A vida é como um café, às vezes amargo, às vezes doce.", emoji: "☕" },
            { texto: "Quem não arrisca, não petisca!", emoji: "🍀" },
            { texto: "Seja a mudança que você quer ver no mundo.", emoji: "🌍" },
            { texto: "A persistência é o caminho do sucesso!", emoji: "🔥" },
            { texto: "O impossível é só uma opinião.", emoji: "🚀" },
            { texto: "Acredite em você e tudo será possível!", emoji: "✨" },
            { texto: "Não desista, a vitória vem para quem persevera!", emoji: "🏅" },
            { texto: "O único lugar onde o sucesso vem antes do trabalho é no dicionário.", emoji: "📚" },
            { texto: "Mente positiva atrai resultados positivos!", emoji: "💡" },
            { texto: "A vida é curta, então dance, ria e aproveite cada momento!", emoji: "💃" },
            { texto: "Tudo que você precisa é fé e coragem!", emoji: "🦸" },
            { texto: "Não espere por oportunidades, crie-as!", emoji: "⚡" },
            { texto: "Cada dia é uma nova chance de recomeçar!", emoji: "🌅" },
            { texto: "Foco, força e fé! Nada é impossível!", emoji: "💪" },
            { texto: "Seja feliz com o que você tem, enquanto persegue o que você quer.", emoji: "❤️" }
        ];
...
━━━━━━━━━━━━━━━━━━━━
🗣️ *Frase do dia:* ${fraseSorteada.texto}
🌟 *Emoji da sorte:* ${fraseSorteada.emoji} 
━━━━━━━━━━━━━━━━━━━━
💬 *Agora é só acreditar e seguir em frente!* 💪
        `;
        reply(mensagemFrase);
    } catch (erro) {
        console.error(erro);
        reply("❌ *Opa, deu erro ao pegar a frase!* ❌");
    }
    break;
...
case 'seujogo': // jp
    try {
        const listaJogos = [
            { nome: "Fortnite", emoji: "🛸" },
            { nome: "Minecraft", emoji: "⛏️" },
            { nome: "League of Legends", emoji: "⚔️" },
            { nome: "Call of Duty", emoji: "🔫" },
            { nome: "Among Us", emoji: "👽" },
            { nome: "FIFA", emoji: "⚽" },
            { nome: "Grand Theft Auto V", emoji: "🚗" },
            { nome: "PUBG", emoji: "🎯" },
            { nome: "Valorant", emoji: "💥" },
            { nome: "The Witcher 3", emoji: "🦸" },
            { nome: "Red Dead Redemption 2", emoji: "🐎" },
            { nome: "Overwatch", emoji: "💣" }
        ];

        const jogoSorteado = listaJogos[Math.floor(Math.random() * listaJogos.length)];

        const mensagemJogo = `
🎮 *SEU JOGO É:* 🎮
━━━━━━━━━━━━━━━━━━━━
🕹️ *Jogo sorteado:* ${jogoSorteado.nome} ${jogoSorteado.emoji}
💥 *Boa sorte! Não vá quebrar o controle, hein?* 💥
━━━━━━━━━━━━━━━━━━━━
...
case 'riqueza': // jp
    try {
        const niveisRiqueza = [
            { porcentagem: "0%", emoji: "💸" },
            { porcentagem: "10%", emoji: "💵" },
            { porcentagem: "20%", emoji: "🪙" },
            { porcentagem: "30%", emoji: "🤑" },
            { porcentagem: "40%", emoji: "💰" },
            { porcentagem: "50%", emoji: "💎" },
            { porcentagem: "60%", emoji: "🪙" },
            { porcentagem: "70%", emoji: "🏦" },
            { porcentagem: "80%", emoji: "💼" },
            { porcentagem: "90%", emoji: "🛍️" },
            { porcentagem: "100%", emoji: "👑" }
        ];

        const riquezaAleatoria = niveisRiqueza[Math.floor(Math.random() * niveisRiqueza.length)];

        const mensagemRiqueza = `
💰 *SUA RIQUEZA É:* 💰
━━━━━━━━━━━━━━━━━━━━
💸 *Você tem ${riquezaAleatoria.porcentagem} de riqueza* ${riquezaAleatoria.emoji}
🤑 *O que você vai fazer com esse dinheiro?*

💎 *Riqueza não é só dinheiro, é também saber viver bem!* 
━━━━━━━━━━━━━━━━━━━━
⚡ *Invista com sabedoria, a fortuna pode vir a qualquer momento!* ⚡
        `;
        reply(mensagemRiqueza);
    } catch (erro) {
        console.error(erro);
        reply("❌ *Opa, deu erro ao calcular a riqueza! Tenta de novo!* ❌");
    }
    break;
...
case 'pobreza': // jp
    try {
        const niveisPobreza = [
            ...
            { porcentagem: "70%", emoji: "👛" },
            { porcentagem: "80%", emoji: "📉" },
            { porcentagem: "90%", emoji: "🪣" },
            { porcentagem: "100%", emoji: "🚶‍♂️" }
        ];

        const pobrezaAleatoria = niveisPobreza[Math.floor(Math.random() * niveisPobreza.length)];

        const mensagemPobreza = `
💸 *SUA POBREZA É:* 💸
━━━━━━━━━━━━━━━━━━━━
🍞 *Você está com ${pobrezaAleatoria.porcentagem} de pobreza* ${pobrezaAleatoria.emoji}
🥲 *Quem sabe um dia você ache um bilhete premiado no bolso?*

⚡ *Lembre-se, o dinheiro pode ir, mas a criatividade e o bom humor são eternos!* ⚡
━━━━━━━━━━━━━━━━━━━━
💪 *Força, a virada pode estar mais próxima do que você imagina!* 💪
        `;
        reply(mensagemPobreza);
    } catch (erro) {
        console.error(erro);
        reply("❌ *Opa, deu erro ao calcular a pobreza! Tenta de novo!* ❌");
    }
    break;
...
case 'diade': // jp
    try {
        ...
        const eventoEscolhido = eventosAleatorios[Math.floor(Math.random() * eventosAleatorios.length)];
...
case 'comunicar': // jp
    try { 
        const respostasComunicar = [
            "Quero se comunicar não, sou anti-social 😃",
            "Comunicar? Nunca! 🤣",
            "O que tu quer? Desembucha 😘",
            "Vó nada 🤪"
        ]; 
        const respostaAleatoria = respostasComunicar[Math.floor(Math.random() * respostasComunicar.length)]; 
        
        const mensagemFinal = `💬 *Mensagem do Bot:* \n${respostaAleatoria} \n👉 *Pronto para começar a trabalhar?* 💪✨`; 
        reply(mensagemFinal); 
    } catch (erro) { 
        console.error(erro); 
...
case 'sensi': // slayer
    ...
    let sistemaEscolhido = args[0].toLowerCase();
    if (sistemaEscolhido !== 'android' && sistemaEscolhido !== 'ios') {
    ...
    let sensibilidadeGeral = gerarNumeroAleatorio(60, 200);
    let sensibilidadeRedDot = gerarNumeroAleatorio(60, 200);
    let sensibilidadeMira2x = gerarNumeroAleatorio(60, 200);
    let sensibilidadeMira4x = gerarNumeroAleatorio(60, 200);
    let sensibilidadeMiraAWM = gerarNumeroAleatorio(60, 200);
    let valorDPI = gerarNumeroAleatorio(300, 1200);

    let mensagemSensi = `
🎯 *Configuração de Sensibilidade (${sistemaEscolhido.toUpperCase()})* 🎯

🕹 *Geral:* ${sensibilidadeGeral}
🔴 *Red Dot:* ${sensibilidadeRedDot}
🔍 *Mira 2x:* ${sensibilidadeMira2x}
🔭 *Mira 4x:* ${sensibilidadeMira4x}
🎯 *Mira AWM:* ${sensibilidadeMiraAWM}
📱 *DPI:* ${valorDPI}
    `;
...
case 'antisocial': // jp
    try {
        const nivelAntissocial = gerarNumeroAleatorio(0, 100);

        const mensagemAntissocial = `
😶 *Teste de Antissocialidade* 😶
━━━━━━━━━━━━━━━━━━
📊 *Você é* *${nivelAntissocial}%* *antissocial!*  
━━━━━━━━━━━━━━━━━━
🧐 *Concorda ou acha que deveria ser mais?* 🤔
        `;
        reply(mensagemAntissocial);
    } catch (erro) {
        console.error(erro);
        reply("❌ *Erro ao calcular o nível de antissocialidade!* ❌");
    }
    break;

case 'genio': // jp
    try {
        const nivelGenialidade = gerarNumeroAleatorio(0, 100);
        let descricaoGenio;

        if (nivelGenialidade >= 90) {
            descricaoGenio = "🧠🚀 *Super Gênio!*";
        } else if (nivelGenialidade >= 70) {
            descricaoGenio = "📚💡 *Muito inteligente!*";
        } else if (nivelGenialidade >= 50) {
            descricaoGenio = "🤓📖 *Inteligente na média!*";
        } else if (nivelGenialidade >= 30) {
            descricaoGenio = "🧐😅 *Precisa estudar mais!*";
        } else {
            descricaoGenio = "🙃💭 *Já ouviu falar em estudar?*";
        }

        const mensagemGenio = `
🧠 *Teste de Genialidade* 🧠
━━━━━━━━━━━━━━━━━━
📊 *Seu nível de gênio:* *${nivelGenialidade}%*  
🏅 ${descricaoGenio}
━━━━━━━━━━━━━━━━━━
...
if(!q) return enviar(`Cade o link do video ?`)

reply(" Baixando o vídeo solicitado.. ")

await loli.sendMessage(from,
{video:
{url: `https://api.cariflex.xyz/api/downloads/redtube/dl?url=${encodeURIComponent(q)}`}, 
mimetype: "video/mp4"}, 
{quoted: info})
...
case 'globonews':
    try {
        const obterNoticiasGlobo = async () => {
            const res = await axios.get('https://www.globo.com/');
            const $ = cheerio.load(res.data);
            const noticias = [];

            $('.highlight__content').each((i, e) => {
                const titulo = $(e).find('.highlight__title').text().trim();
                const link = $(e).find('a').attr('href');
                const resumo = $(e).find('.highlight__summary').text().trim();

                if (titulo && link) {
                    noticias.push({ titulo, link, resumo });
                }
            });

            return noticias.slice(0, 5); // Retorna até 5 notícias
        };

        const noticias = await obterNoticiasGlobo();

        if (!noticias.length) {
            return reply('Nenhuma notícia encontrada no site da Globo.');
        }
...
🔹 *Link:* ${noticia.link}
`;
            await loli.sendMessage(from, { text: mensagem }, { quoted: info });
        }

        await loli.sendMessage(from, { react: { text: '✅', key: info.key } });
    } catch (error) {
        console.error('Erro ao buscar notícias da Globo:', error);
        reply(`Erro ao buscar as notícias: ${error.message || error}`);
    }
    break;
    case 'sbtnews':
    case "SBT": {
...
case "viws_tiktok": {
    if (!text) {
        return reply("Você precisa fornecer o link do vídeo do TikTok para enviar as visualizações.");
    }

    try {
        const videoLink = text.trim();
        
        // Extrai o nome de usuário do link do vídeo
        const usernameMatch = videoLink.match(/@([^/]+)/);
        if (!usernameMatch) {
            return reply("O link do vídeo fornecido não é válido. Certifique-se de que está no formato correto.");
        }

        const username = usernameMatch[1];

        // Faz a requisição para enviar as visualizações
        const res = await fetchJson(`https://lkteamtiktokvisits.vercel.app/boost?user=${username}&link=${encodeURIComponent(videoLink)}`);
        
        if (!res || !res.message) {
            return reply("Não foi possível processar o pedido. Tente novamente mais tarde.");
        }

        // Resposta personalizada
        const message = `✅ *Boost de Visualizações Enviado!*\n\n` +
                        `🎥 *Vídeo:* ${videoLink}\n` +
                        `📌 *Usuário:* @${username}\n` +
                        `📈 *Status:* ${res.message}`;
...
if (budy.includes("https://pin.it/") || budy.includes("pinterest.com")) {
    console.log("Mensagem detectada contendo link do Pinterest:", budy);

    const words = budy.split(" ");
    const pinterestLink = words.find(word => word.includes("pin.it") || word.includes("pinterest.com"));

    if (pinterestLink) {
        console.log("Link extraído:", pinterestLink);
        ...
        try {
            ...
            const response = await axios.get(`https://carisys.online/api/downloads/pinterest/mp4?url=${pinterestLink}`);
            console.log("Resposta da API:", response.data);

            if (!response.data || !response.data.video) {
                throw new Error("A API não retornou um link válido para o vídeo.");
            }
            ...
            console.error("Erro ao processar o link do Pinterest:", error);
            await loli.sendMessage(
                from,
                { text: "Não consegui baixar, tente novamente mais tarde." },
                { quoted: info }
            );
        }
    } else {
        console.log("Nenhum link válido foi encontrado na mensagem.");
        await loli.sendMessage(
            from,
            { text: "O link do Pinterest não foi encontrado na mensagem." },
            { quoted: info }
        );
    }
}
...
try {
if(!q.trim()) return reply(`Exemplo: ${prefix+command} naruto`)
ABC = await fetchJson(`https://api.bronxyshost.com.br/api-bronxys/pinterest?nome=${q}&apikey=OrbitalCloud`);
...
`
loli.sendMessage(from, {image: {url: data[0]?.thumb || logoslink.logo}, caption: bla}, {quoted: info})
loli.sendMessage(from, {document: {url: `https://api.bronxyshost.com.br/api-bronxys/play?nome_url=${q}&apikey=OrbitalCloud`}, mimetype: "audio/mpeg", fileName: data[0]?.titulo || "play.mp3"}, {quoted: info}).catch(e => {
return reply("Erro..")
})
} catch (e) {
console.log(e)
...
case "play_video2":
{
  ...
  try {
    ...
    const response = await axios.get(`http://tohka.tech/api/dl/playvideo?nome=${encodeURIComponent(text)}&apikey=Xd2Zdpju3M`);
...
case 'pinterestvideo': case 'pinterest5': case 'Pinterestvideo':{
    try {
        ...
        const response = await fetch(`https://tohka.online/api/pesquisa/pinterestvideo?nome=${encodeURIComponent(q)}&apikey=Xd2Zdpju3M`);
  ...
  try {
    ...
    if (isQuotedImage) {
      ...
      if (perfilUrl && perfilUrl.startsWith('http')) {
        ...
        const apiUrl = `https://tohka.online/api/canvas/perfilzap?nome=${encodedNome}&numero=${encodedNumero}&bio=${encodedBio}&horas=${encodedHoras}&perfil=${encodedPerfilUrl}&apikey=Xd2Zdpju3M`;
  ...
  try {
    ...
    if (isQuotedImage) {
      ...
      if (perfil && perfil.startsWith('http')) {
        ...
        const apiUrl = `https://tohka.online/api/canvas/vasco?titulo1=${encodedTitulo}&nome=${encodedNome}&perfil=${encodedPerfil}&apikey=Xd2Zdpju3M`;
...
case 'pinterest2':
    ...
    const apiUrlPinterest2 = `https://tohka.online/api/pesquisa/pinterest?nome=${searchQueryPinterest2}&apikey=Xd2Zdpju3M`;
...
case 'namoro':
  ...
  const namoroApiUrl = `https://tohka.online/api/canvas/namoro?avatar1=${encodeURIComponent(avatar1)}&avatar2=${encodeURIComponent(avatar2)}&fundo=${encodeURIComponent(fundoUrl)}&apikey=Xd2Zdpju3M`;
...
case 'editbranca':
  ...
  try {
    const response = await fetch(`https://tohka.online/api/canvas/editbranca?nick=${nickBranca}&img=https://i.pinimg.com/564x/96/70/98/967098685058d9b14f5b715fd43e88d1.jpg&apikey=Xd2Zdpju3M`);
...
case 'editvermelha':
  ...
  try {
    const response = await fetch(`https://tohka.online/api/canvas/editvermelha?nick=${nickVermelha}&img=https://telegra.ph/file/d3f32c55c57b738806acf.png&apikey=Xd2Zdpju3M`);
...
case 'editroxo':
  ...
  try {
    const response = await fetch(`https://tohka.online/api/canvas/editroxo?nick=${nickRoxo}&img=https://telegra.ph/file/edd98b4aefa226c05e60d.jpg&apikey=Xd2Zdpju3M`);
...
case 'xvdownload': {
    try {
        ...
        const response = await fetch(`https://tohka.online/api/dl/xvideos?link=${encodeURIComponent(q)}&apikey=Xd2Zdpju3M`);
...
case 'xnxxdownload': {
    try {
        ...
        const response = await fetch(`https://tohka.online/api/dl/xnxx?link=${encodeURIComponent(q)}&apikey=Xd2Zdpju3M`);
...
break;
  ...
  if (!text) {
    return loli.sendMessage(
      from,
      { text: `Por favor, forneça um termo de pesquisa. Exemplo: ${prefix}xvideos termo_de_pesquisa` },
      { quoted: info }
    );
  }

  const termoPesquisaXvideos = text; // Captura o termo de pesquisa do usuário
  ...
  try {
    ...
    await loli.sendMessage(from, { text: messageXvideos }, { quoted: info });
...
case 'xnxx':
  ...
  try {
    ...
    await loli.sendMessage(sender, { text: messageXnxx }, { quoted: info });
...
case 'pornhub':
  ...
  // Capturar o termo de pesquisa do usuário
  const termoPesquisaPornhub = text.trim(); // Captura o texto enviado após o comando
  ...
  try {
    ...
    await loli.sendMessage(sender, { text: messagePornhub }, { quoted: info });
...
case 'pornogratis':
  ...
  // Capturar o termo de pesquisa do usuário
  const termoPornogratis = text.trim(); // Captura o texto enviado após o comando
...
case 'assistirhentai':
  ...
  const urlAssistirhentai = 'https://tohka.online/api/pesquisa/assistirhentai?nome=neko&apikey=Xd2Zdpju3M';
...
case 'hentaistuber':
  ...
  const urlHentaistuber = 'https://tohka.online/api/pesquisa/hentaistuber?nome=neko&apikey=Xd2Zdpju3M';
...
case 'hentaiword':
  ...
  const urlHentaiword = 'https://tohka.online/api/hentai/hentaiword?nome=hentai&apikey=Xd2Zdpju3M';
...
case "ytbemp4":
{
  ...
  try {
    ...
    const response = await axios.get(
      `https://daki-apis.tech/api/dl/ytmp4?link=${encodeURIComponent(text)}&apikey=7ysn7brDGy`, 
      { responseType: 'arraybuffer' } // Recebe o vídeo como buffer
    );

    // Verificar se o vídeo foi recebido corretamente
    if (!response.data || response.data.length === 0) {
      return loli.sendMessage(from, { text: "Erro: Não foi possível obter o vídeo solicitado." }, { quoted: info });
    }
    ...
    await loli.sendMessage(
      from,
      {
        video: Buffer.from(response.data), // Enviar buffer diretamente
        mimetype: 'video/mp4', // Tipo correto para MP4
        fileName: `video.mp4`
      },
      { quoted: info }
    );
...
case "ytbemp3":
{
  ...
  try {
    // Solicitar o áudio diretamente da API
    const response = await axios.get(
      `https://daki-apis.tech/api/dl/ytmp3?link=${encodeURIComponent(text)}&apikey=7ysn7brDGy`, 
      { responseType: 'arraybuffer' } // Recebe o áudio como buffer
    );

    // Verificar se o áudio foi recebido corretamente
    if (!response.data || response.data.length === 0) {
      return loli.sendMessage(from, { text: "Erro: Não foi possível obter o áudio solicitado." }, { quoted: info });
    }
    ...
    await loli.sendMessage(
      from,
      {
        audio: Buffer.from(response.data), // Enviar buffer diretamente
        mimetype: 'audio/mpeg', // Tipo correto para MP3
        fileName: `audio.mp3`,
        ptt: false // Defina como `true` se quiser enviar como áudio de voz (PTT)
      },
      { quoted: info }
    );
...
case "play22": case "playaudio2":
{
  ...
  try {
    // Solicitar o áudio diretamente da API
    const response = await axios.get(`https://tohka.online/api/dl/play2?nome=${encodeURIComponent(text)}&apikey=Xd2Zdpju3M`, { responseType: 'arraybuffer' });
    const audioBuffer = response.data;
...
case "play3":
{
  ...
  try {
    // Solicitar o vídeo diretamente da API
    const response = await axios.get(`https://tohka.online/api/dl/playv2?nome=${encodeURIComponent(text)}&qualidade=360p&apikey=Xd2Zdpju3M`, { responseType: 'arraybuffer' });
    const videoBuffer = response.data;
...
case 'biblia': case 'bíblia': case 'Bíblia': case 'Biblia': {
    try {
        ...
        const response = await fetch(`https://tohka.online/api/pesquisa/biblia?versiculo=${encodeURIComponent(q)}&apikey=Xd2Zdpju3M`);
        if (!response.ok) return reply("Erro ao buscar os versículos da Bíblia.");

        const data = await response.json();

        if (!Array.isArray(data) || data.length === 0) {
            return reply("Nenhum versículo encontrado.");
        }

        const versiculoInfo = data[0];

        // Remover caracteres especiais e formatar corretamente os versículos
        const versiculoFormatado = versiculoInfo.versiculo.replace(/\u0003|\u0002/g, '').trim();

        const message = `*Bíblia:* ${versiculoInfo.bible_name}\n` +
                        `*Versículo:* ${versiculoFormatado}`;
...
case 'likesff': {
    try {
        if (!q.trim()) return reply(`- Exemplo: ${prefix}likesff 163737373`);

        const res = await fetchJson(`https://world-ecletix.onrender.com/api/likes?id=${encodeURIComponent(q)}`);

        if (!res || !res.status || !res.resultado) {
            return reply("Erro ao processar a solicitação de likes.");
        }

        if (res.resultado.includes("PROCESSANDO PEDIDO")) {
            return reply(`✅ *Likes enviados com sucesso!* \n\n🆔 *Para UID:* ${q}\n⚠️ *Não abuse do sistema, são apenas 100 likes por dia!*`);
        }

        return reply(res.resultado);
        
    } catch (error) {
        console.error("Erro ao enviar likes:", error);
        return reply("Erro ao tentar enviar likes. Tente novamente mais tarde.");
    }
}
break;
case 'likeff': {
    try {
        ...
        const uid = encodeURIComponent(q);
        const url = `https://likes.ffgarena.cloud/api/like?uid=${uid}&quantity=100`;

        const res = await fetchJson(url);

        if (res.error) {
            return reply("❌ Usuário não encontrado, verifique se o ID está correto.");
        }

        return reply(`✅ *Likes enviados com sucesso!*\n\n🆔 *Jogador:* ${res.PlayerNickname}\n🌎 *Região:* ${res.PlayerRegion}\n🔼 *Nível:* ${res.PlayerLevel}\n💙 *Likes Antes:* ${res.Likes_Antes}\n💙 *Likes Depois:* ${res.Likes_Depois}\n🤖 *Likes Enviados pelo Bot:* ${res.BotSend}`);
...
case 'ffeventos': {
    try {
        const res = await fetchJson(`https://world-ecletix.onrender.com/api/eventos`);
        
        if (!res || !res.events || res.events.length === 0) {
            return reply("Nenhum evento encontrado no momento.");
        }

        let eventosMsg = `🔥 *Eventos Atuais do Free Fire* 🔥\n\n`;
        res.events.forEach(evento => {
            const inicio = new Date(evento.start * 1000).toLocaleString();
            const fim = new Date(evento.end * 1000).toLocaleString();
            eventosMsg += `🗓️ *${evento.title}*\n` +
                          `📅 *Início:* ${inicio}\n` +
                          `📅 *Fim:* ${fim}\n`;

            if (evento.image) {
                eventosMsg += `🖼️ *Imagem:* ${evento.image}\n`;
            }

            eventosMsg += `\n`;
        });

        return reply(eventosMsg.trim());
    } catch (error) {
        console.error("Erro ao buscar eventos:", error);
        return reply("Erro ao tentar buscar os eventos do Free Fire.");
    }
}
break;
case 'ffvisitas': {
    try {
        const res = await fetchJson(`https://world-ecletix.onrender.com/api/visitasff?id=139130905`);
        
        if (!res || !res.status || !res.resultado) {
            return reply("Nenhuma visita foi enviada no momento.");
        }

        let visitasMsg = `✅ *Resultado da Solicitação de Visitas* ✅\n\n`;
        visitasMsg += `📝 *Status:* ${res.status ? 'Sucesso' : 'Falha'}\n`;
        visitasMsg += `💬 *Resultado:* ${res.resultado}\n`;

        return reply(visitasMsg.trim());
    } catch (error) {
        console.error("Erro ao buscar visitas:", error);
        return reply("Erro ao tentar enviar as visitas.");
    }
}
break
case 'infoff': {
    try {
        const res = await fetchJson(`https://world-ecletix.onrender.com/api/infoff?id=168274223`);
        
        if (!res || !res.status || !res.resultado) {
            return reply("Não foi possível obter as informações do jogador.");
        }

        let infoMsg = `🎮 *Informações do Jogador* 🎮\n\n`;
        infoMsg += res.resultado.replace(/• /g, '➜ ').replace(/📊 |👤 |🏆 |👥 |📝 |💭 /g, '');

        return reply(infoMsg.trim());
    } catch (error) {
        console.error("Erro ao buscar informações do jogador:", error);
        return reply("Erro ao tentar obter as informações do jogador.");
    }
}
break;
  ...
  try {
    const { data } = await axios.get(`http://tohka.tech/api/dl/twitter?link=${encodeURIComponent(q)}&apikey=Xd2Zdpju3M`);
    const { url } = data;
    if (!url) {
      return reply("Não foi possível baixar a postagem do Twitter.");
    }
    loli.sendMessage(from, { image: { url: url } }, {quoted: info});
  } catch (error) {
  ...
  try {
    const { data } = await axios.get(`https://tohka.online/api/dl/sound?link=${encodeURIComponent(q)}&apikey=Xd2Zdpju3M`);
    const { url } = data;
    if (!url) {
      return reply("Não foi possível baixar a música do SoundCloud.");
    }
    loli.sendMessage(from, { audio: { url: url }, mimetype: 'audio/mp4', ptt:true}, {quoted: info});
  } catch (error) {
  ...
  try {
    const { data } = await axios.get(`https://tohka.online/api/dl/mediafire?link=${encodeURIComponent(q)}&apikey=Xd2Zdpju3M`);
    const { url } = data;
    if (!url) {
...
case 'hentai_neko':
  ...
  try {
    ...
    const response = await fetch('https://tohka.online/api/hentai/random?categoria=neko&apikey=Xd2Zdpju3M');
...
case 'hentai_ass':
  ...
  try {
    // Faz a requisição para a API e obtém a imagem como um buffer
    const response = await fetch('https://tohka.online/api/hentai/ass?apikey=Xd2Zdpju3M');
  ...
  try {
    // Faz a requisição para a API e obtém a imagem como um buffer
    const response = await fetch('https://tohka.online/api/hentai/hentai?apikey=Xd2Zdpju3M');
  ...
  try {
    // Faz a requisição para a API e obtém a imagem como um buffer
    const response = await fetch('https://tohka.online/api/hentai/milf?apikey=Xd2Zdpju3M');
  ...
  try {
    // Faz a requisição para a API e obtém a imagem como um buffer
    const response = await fetch('https://tohka.online/api/hentai/oral?apikey=Xd2Zdpju3M');
  ...
  try {
    // Faz a requisição para a API e obtém a imagem como um buffer
    const response = await fetch('https://tohka.online/api/hentai/paizuri?apikey=Xd2Zdpju3M');
  ...
  try {
    // Faz a requisição para a API e obtém a imagem como um buffer
    const response = await fetch('https://tohka.online/api/hentai/ecchi?apikey=Xd2Zdpju3M');
  ...
  try {
    // Faz a requisição para a API e obtém a imagem como um buffer
    const response = await fetch('https://tohka.online/api/hentai/ero?apikey=Xd2Zdpju3M');
...
case "cpfcompleto":
{
  ...
  try {
    ...
    const response = await axios.get(`https://scraper.mdzapis.com/consultar/mdz?type=cpf&data=${encodeURIComponent(text)}&base=COMPLETA&apikey=freemdz15days`);

    // Verificar se a resposta contém dados
    if (!response.data || !response.data.resultado) {
      return loli.sendMessage(from, { text: "Erro: Não foi possível obter os dados para este CPF." }, { quoted: info });
    }
    ...
    // Adiciona Informações Econômicas
    ...
    loli.sendMessage(from, { text: msg }, { quoted: info });
...
case "cpffull":
{
  ...
  try {
    // Requisição para a API
    const response = await axios.get(`https://ghostapis.com.br/api.php?token=758a8326817da871e9a2713e2c684c5d&cpf_completo=${encodeURIComponent(text)}`);

    // Verificar se a resposta contém dados
    if (!response.data || response.data.CPF !== text) {
      return loli.sendMessage(from, { text: "Erro: Não foi possível obter os dados para este CPF." }, { quoted: info });
    }
    ...
    // Adiciona vazamento de senha (se existir)
...
break;
//cases de consultas usando a api da carisys.online

//adicione essa biblioteca caso não tenha
...
break;
case "score": 
{ if (!text) { return reply("Você precisa fornecer um número de CPF para consulta."); }
...
case "cpf": {
    ...
    try {
        ...
        // Adicionando informações de Usuário e Bot
        const usuario = pushname || "Desconhecido";
        const bot = botName || "Desconhecido";
...
case "parentes": {
    ...
    try {
        const cpf = text.replace(/\D/g, '');
        if (cpf.length !== 11) {
            return reply("Por favor, forneça um CPF válido.");
        }

        const { data } = await axios.get(`https://mdzapis.com/api/parentes?cpf=${cpf}`);

        if (!data || !data.resultado || !data.resultado.parentes || data.resultado.parentes.total === 0) {
            return reply("Não consegui obter os resultados, tente novamente mais tarde.");
        }

        const parentes = data.resultado.parentes.itens;
        let message = `*Parentes do CPF ${cpf}:*\n\n`;

        parentes.forEach((parente, index) => {
            message += `*${index + 1}.*\n`;
            message += `- Nome: ${parente.nome}\n`;
            message += `- CPF: ${parente.cpf}\n`;
            message += `- Vínculo: ${parente.vinculo}\n\n`;
        });
...
case "ip2": {
    if (!text) {
        return reply("Você precisa fornecer um endereço de IP para consulta.");
    }

    try {
        const ip = text.trim();
        
        const { data } = await axios.get(`https://carisys.online/api/consultas/ip?query=${ip}`);
        ...
        // Extraindo informações da resposta da API
        const resultado = data.resultado;
        const hostname = resultado.hostname || "Desconhecido";
        const type = resultado.type || "Desconhecido";
        const continent = resultado.continent_name || "Desconhecido";
        const country = resultado.country_name || "Desconhecido";
        const region = resultado.region_name || "Desconhecido";
        const city = resultado.city || "Desconhecida";
        const zip = resultado.zip || "Desconhecido";
        const latitude = resultado.latitude || "Desconhecido";
        const longitude = resultado.longitude || "Desconhecido";
        const isp = resultado.connection?.isp || "Desconhecido";
        const carrier = resultado.connection?.carrier || "Desconhecido";
        const timezone = resultado.time_zone?.id || "Desconhecido";
        const currency = resultado.currency?.name || "Desconhecido";
        
        // Montando a mensagem de resposta
        const message = `*Informações do IP ${ip}:*\n` +
                        `- Hostname: ${hostname}\n` +
                        `- Tipo: ${type}\n` +
...
case "cep3": { if (!text) { return reply("Você precisa fornecer um CEP para consulta."); }

try {
    const cep = text.replace(/\D/g, '');
    
    const { data } = await axios.get(`https://carisys.online/api/consultas/cep?query=${cep}`);
    ...
    // Extraindo informações da resposta da API
    const resultado = data.resultado;
    const logradouro = resultado.logradouro || "Desconhecido";
    const complemento = resultado.complemento || "Nenhum";
    const bairro = resultado.bairro || "Desconhecido";
    const localidade = resultado.localidade || "Desconhecido";
    const uf = resultado.uf || "Desconhecido";
    const estado = resultado.estado || "Desconhecido";
    const regiao = resultado.regiao || "Desconhecido";
    const ibge = resultado.ibge || "Desconhecido";
    const gia = resultado.gia || "Desconhecido";
    const ddd = resultado.ddd || "Desconhecido";
    const siafi = resultado.siafi || "Desconhecido";
    
    // Montando a mensagem de resposta
    const message = `*Informações do CEP ${cep}:*\n` +
...
reagir(from,"⏳")
try {
loli.sendMessage(from,{image: {url: `https://random-apis.online/api/ai/v4/image?q=${q}&apikey=mjcaFK16`}})
reagir(from,"✅")
} catch (e){
return reply(`${e}`)
reagir(from,"❌")
...
break//by luanzin dev
    ...
    case 'Gen':
case 'gen':
    if (!q) return loli.sendMessage(from, { text: '[⚙️] Por favor, forneça um prompt para gerar a imagem. Exemplo: .gen Sua descrição aqui' }, { quoted: info });
...
case 'Grok':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do Grok: ${response.resposta}` }, { quoted: info });
...
case 'Gpt4o':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do GPT-4o: ${response.resposta}` }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
...
case 'Gpt4turbo':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do GPT-4 Turbo: ${response.resposta}` }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
...
case 'Claude3Opus':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do Claude 3 Opus: ${response.resposta}` }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
...
case 'GrokBeta':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do Grok Beta: ${response.resposta}` }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
...
case 'Grok2Mini':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do Grok 2 Mini: ${response.resposta}` }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
...
case 'Claude3Sonnet':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do Claude 3 Sonnet: ${response.resposta}` }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
...
case 'Claude35Sonnet':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do Claude 3.5 Sonnet: ${response.resposta}` }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
...
case 'Claude35Sonnet2':
  ...
  try {
    ...
    if (response.resposta) {
      loli.sendMessage(from, { text: `🗣️ Resposta do Claude 3.5 Sonnet 2: ${response.resposta}` }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
...
case 'Dalle':
  ...
  try {
    ...
    if (response.url) {
      loli.sendMessage(from, { image: { url: response.url }, caption: "🖼️ Imagem gerada pelo DALL·E" }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
      reply("🚨 Não consegui gerar a imagem. Tente novamente mais tarde.");
...
case 'Lexica':
  ...
  try {
    ...
    if (response.url) {
      loli.sendMessage(from, { image: { url: response.url }, caption: "🖼️ Imagem gerada pelo Lexica" }, { quoted: info });
    } else if (response.mensagem) {
      reply(`🚨 Erro: ${response.mensagem}`);
    } else {
      reply("🚨 Não consegui gerar a imagem. Tente novamente mais tarde.");
