...
if (budy.includes("pin.it") || budy.includes("pinterest.com")) {
  // Extrai o link diretamente da mensagem
  const words = budy.split(" "); // Divide o texto por espaços
  ...
  if (pinterestLink) {
    try {
      ...
      await loli.sendMessage(
        from,
        {
          video: { url: `https://carisys.online/api/downloads/pinterest/mp4?url=${pinterestLink}` },
          mimetype: "video/mp4"
        },
        { quoted: info }
      );
    ...
    await loli.sendMessage(
      from,
      { text: "O link não é válido na sua mensagem." },
      { quoted: info }
    );
  }
}
...
case 'Playvideo':
  try {
    if (!q) return reply('🚨 Coloque o nome ou o link do vídeo.');

    // Verifica se a entrada é um link ou um termo de busca
    const isLink = q.startsWith('http://') || q.startsWith('https://');

    let firstResult;
    if (isLink) {
      // Se for um link, cria um objeto simulado com as informações básicas
      firstResult = { 
        title: 'Link fornecido', 
        duration: 'Não disponível', 
        channel: 'Não disponível', 
        views: 'Não disponível', 
        thumbnail: 'https://example.com/default_thumbnail.jpg', // Miniatura padrão
        link: q.trim(),
      };
    } else {
      ...
      // Pegando a primeira informação do array formattedVideos
      firstResult = data.formattedVideos[0];
    }
...
case 'Play':
case 'play':
  if (!q) return loli.sendMessage(from, { text: '🚨 Informe o nome ou o link da música.' }, { quoted: info });
...
case 'playlink': case 'ytbmp3': {
    try {
        if (!firstResult || !firstResult.link) {
            return reply("Não foi possível encontrar o link. Certifique-se de enviar uma consulta válida.");
        }
        ...
        await loli.sendMessage(
            from,
            {
                audio: { url: `https://carisys.online/api/downloads/youtube/mp3-3?url=${encodeURIComponent(firstResult.link)}` },
                mimetype: "audio/mpeg",
                fileName: firstResult.title || "play.mp3",
            },
            { quoted: info }
        ).catch(e => {
            console.error("Erro ao enviar o áudio:", e);
            return reply("Erro ao tentar baixar a música.");
        });
    } catch (e) {
        console.error("Erro geral:", e);
        return reply("Seja mais específico. Não foi possível encontrar com apenas isso.");
    }
}
break;

case 'videolink': case 'ytbmp4': case 'clipelink': {
    try {
        if (!q.trim()) {
            return reply(`- Exemplo: ${prefix}videolink nome da música\nA música será baixada. Escolha entre áudio ou vídeo. Caso não funcione, o vídeo pode estar protegido pelo YouTube.`);
        }
        ...
        await loli.sendMessage(
            from,
            {
                video: { url: `https://carisys.online/api/downloads/youtube/mp4?url=${encodeURIComponent(firstResult.link)}` },
                mimetype: "video/mp4",
