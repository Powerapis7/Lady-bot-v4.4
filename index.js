...
case 'playlink2': case 'ytbmp32': {
    try {
        ...
        loli.sendMessage(from, { audio: { url: `https://carisys.online/api/downloads/youtube/mp3-3?url=${encodeURIComponent(firstResult.link)}` }, mimetype: "audio/mpeg", fileName: firstResult.title || "play.mp3" }, { quoted: info }).catch(e => {
            return reply("Erro ao tentar baixar a música.");
        });
    } catch (e) {
        console.log(e);
...
case 'videolink': case 'ytbmp4': case 'clipelink': {
    try {
        ...
        // Baixando o vídeo
        loli.sendMessage(from, {video: {url: `https://carisys.online/api/downloads/youtube/mp4?url=${encodeURIComponent(firstResult.link)}`}, mimetype: "video/mp4", fileName: firstResult.title || "video.mp4"}, {quoted: info}).catch(e => {
